...
def _load_cached_clients(network, cache, normalize_mac):
    key = cache_key(network, normalize_mac)
    existing = [
        dict(client)
        for client in cache.get(key, [])
    ...
    clients_by_identity = {
        client.get('mac') or client.get('ip'): {
            **client,
            'currently_visible': (
                bool(client.get('network_scan_scoped'))
                if client.get('network_scan_scoped')
                else False
            ),
        }
        for client in existing
    ...
    return key, known_identities, clients_by_identity


def _merge_observed_clients(
    network,
    clients_by_identity,
    normalize_mac,
    lookup_manufacturer,
    ...
    for raw_client in network.get('clients') or []:
        ...
        looked_up_manufacturer = (
            lookup_manufacturer(mac) if lookup_manufacturer and mac else None
        )
        client['manufacturer'] = _known_manufacturer(
        ...
        if not identity:
            continue
        previous = clients_by_identity.get(identity, {})
        ...
        clients_by_identity[identity] = {**previous, **client}


def _matching_identity(device, clients_by_identity):
    possible_identities = [
        value for value in (device.get('mac'), device.get('ip')) if value
    ]
    return next(
        (
            identity
            for identity in possible_identities
            if identity in clients_by_identity
        ),
        None,
    )


def _inventory_client_update(
    device,
    previous,
    lookup_manufacturer,
    now,
    ...
    resolved_manufacturer = _known_manufacturer(
        ...
        (
            lookup_manufacturer(device.get('mac'))
            if lookup_manufacturer and device.get('mac')
            else None
        ),
        'Unknown',
    )
    return {
        **previous,
        ...
        'discovery_methods': (
            device.get('discovery_methods') or device.get('sources', [])
        ),
        ...
        'network_first_seen': (
            previous.get('network_first_seen')
...
def _merge_inventory_clients(
    ...
    for device in inventory_records():
        if interface_name and interface_name not in set(device.get('interfaces') or []):
            continue
        if device.get('is_control_traffic') or not (
            device.get('ip') or device.get('mac')
        ):
            continue

        matched_identity = _matching_identity(device, clients_by_identity)
        ...
        previous = clients_by_identity.get(matched_identity, {})
        clients_by_identity[matched_identity] = _inventory_client_update(
            device,
            previous,
            lookup_manufacturer,
            now,
...
def _decorate_client(
    ...
    client['network_known_state'] = (
        'Known'
        if identity in known_identities or (now - first_seen) > 86400
        else 'New'
    )
    ...
    client['display_name'] = client_display_label(
        network,
        client,
        labels,
        normalize_mac,
...
def _partition_clients(clients_by_identity):
    disappeared = sort_clients(
        [
            client
            for client in clients_by_identity.values()
            if not client.get('currently_visible')
            and client.get('source') != 'inventory'
        ]
    )
    visible = sort_clients(
        [
            client
            for client in clients_by_identity.values()
            if client.get('currently_visible')
            or client.get('source') == 'inventory'
        ]
    )
    return visible, disappeared


def merge_network_clients(
    network,
    cache,
    labels,
    normalize_mac,
    inventory_records,
    lookup_manufacturer=None,
):
    """Persist and merge wireless clients plus inventory devices for a network view."""
    now = time.time()
    key, known_identities, clients_by_identity = _load_cached_clients(
        network,
        cache,
        normalize_mac,
    )
    _merge_observed_clients(
        network,
        clients_by_identity,
        normalize_mac,
        lookup_manufacturer,
    ...
    inventory_devices = _merge_inventory_clients(
        network,
        clients_by_identity,
        inventory_records,
        lookup_manufacturer,
    ...
    for identity, client in clients_by_identity.items():
        _decorate_client(
            network,
            identity,
            client,
    ...
    clients, disappeared_clients = _partition_clients(clients_by_identity)
