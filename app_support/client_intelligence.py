...
def enrich_ip_client_display_name(identifier, device=None):
    ...
    with deps.device_inventory_lock:
        ...
        existing_record.update({key: value for key, value in updates.items() if value})
...
def client_intelligence_profile(identifier, active_probe=False):
    ...
    detected_names = [
        ...
        device.get('detected_display_name'),
        *observed_names,
    ]
...
def client_profile_export(identifier):
    ...
    related_evidence = [
        item
        for item in deps.evidence_records()
        if str(item.get('device') or '') in {
            str(host),
            str(device.get('mac') or ''),
...
def client_relationship_map(identifier):
    ...
    for iface in device.get('interfaces', []):
        ...
        links.append({
            'source': f'client:{host}',
            'target': f'interface:{iface}',
            'label': 'seen on',
        })
